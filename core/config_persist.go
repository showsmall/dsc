...
func (m *Manager) persistRemovalLocked(name string) error {
	...
	if changed {
		m.logger.Info("removed core persisted", "name", name, "config", m.persistConfigPath())
	}
	return nil
}

// setRootField 在根映射中设置/创建标量字段（保留其余内容与注释）。
// 整数值写为 !!int 节点，保证读回时能 unmarshal 进 int 字段。
func setRootField(root *yaml.Node, key, value string) {
	tag := "!!str"
...
// isIntegerLiteral 判断字符串是否为合法整数字面量（可带负号）。
...
// SetHistoryInjectionConfig 把 /settings history 的值持久化到 config.yaml 的
// history_injection 字段（保留文件其余内容与注释）；启动时宿主经
// DSC_HISTORY_INJECTION 下发给 agent，跨会话/重启生效。
//
// 编码（config 值）：-1 禁止（不注入历史）、0 未定义（默认不限制）、>0 启用并注入
// 最近 N 条。入参 count 为 agent 内部语义（-1 不限制、0 不注入、N 条），此处转换：
//
//	agent 0（off）        → config -1（禁止）
//	agent -1（unlimited） → config 0（未定义/默认不限制）
//	agent N（N 条）       → config N（启用）
func (m *Manager) SetHistoryInjectionConfig(count int) error {
	encoded := count
	switch count {
	case 0:
		encoded = -1
	...
	doc, err := readConfigDocumentNode(path)
	if err != nil {
		return fmt.Errorf("settings: read config: %w", err)
	}
	root := configMappingNode(doc)
	if doc == nil {
		doc = &yaml.Node{Kind: yaml.DocumentNode, Content: []*yaml.Node{root}}
	}
	setRootField(root, "history_injection", strconv.Itoa(encoded))
	if err := saveConfigNode(path, doc); err != nil {
		return fmt.Errorf("settings: persist config: %w", err)
	}
	return nil
}
