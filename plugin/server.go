...
// CoreProtocolVersion is the ProtocolVersion of the core system itself.
...
// handshake before starting a core connection. This is embedded by
// both ServeConfig and ClientConfig.
//
// In practice, the core host creates a HandshakeConfig that is exported
// and plugins then can easily consume it.
type HandshakeConfig struct {
	// ProtocolVersion is the version that clients must match on to
	// agree they can communicate. This should match the ProtocolVersion
	// set on ClientConfig when using a core.
	// This field is not required if VersionedPlugins are being used in the
	// Client or Server configurations.
	ProtocolVersion uint

	// MagicCookieKey and value are used as a very basic verification
	// that a core is intended to be launched. This is not a security
...
// PluginSet is a set of plugins provided to be registered in the core
// server.
...
type ServeConfig struct {
	...
	// given server options. The server options populated by go-core will
	// be for TLS if set. You may modify the input slice.
	//
	// Note that the grpc.Server will automatically be registered with
	// the gRPC health checking service. This is not optional since go-core
	...
	// Test, if non-nil, will put core serving into "test mode". This is
	// meant to be used as part of `go test` within a core's codebase to
	// launch the core in-process and output a ReattachConfig.
	//
	...
	//   * Stdout/stderr will receive core reads and writes
	//   * Connection information will not be sent to stdout
	//
	Test *ServeTestConfig
}

// ServeTestConfig configures core serving for test mode. See ServeConfig.Test.
type ServeTestConfig struct {
	// Context, if set, will force the core serving to end when cancelled.
	...
	// kill signal. For tests, we need to kill the core serving routinely
	// and this provides a way to do so.
	//
	// If you want to wait for the core process to close before moving on,
	...
	// core client to attach to this core.
	...
	// may be a period of time between canceling the context and the core
...
// protocolVersion determines the protocol version and core set to be used by
...
func protocolVersion(opts *ServeConfig) (int, Protocol, PluginSet) {
	...
	if vs := os.Getenv("PLUGIN_PROTOCOL_VERSIONS"); vs != "" {
		for _, s := range strings.Split(vs, ",") {
			...
			if err != nil {
				fmt.Fprintf(os.Stderr, "server sent invalid core version %q", s)
				continue
			}
	...
	if coreSet != nil {
		opts.VersionedPlugins[protoVersion] = coreSet
	}
	...
	for _, version := range versions {
		...
		coreSet = opts.VersionedPlugins[version]
		...
		for _, clientVersion := range clientVersions {
			if clientVersion == protoVersion {
				return protoVersion, protoType, coreSet
			}
		}
	}
...
//
// Serve doesn't return until the core is done being executed. Any
...
func Serve(opts *ServeConfig) {
	...
	if opts.Test == nil {
		...
		if opts.MagicCookieKey == "" || opts.MagicCookieValue == "" {
			fmt.Fprintf(os.Stderr,
				"Misconfigured ServeConfig given to serve this core: no magic cookie\n"+
					"key or value was set. Please notify the core author and report\n"+
					"this as a bug.\n")
			exitCode = 1
			return
		}
		...
		if os.Getenv(opts.MagicCookieKey) != opts.MagicCookieValue {
			fmt.Fprintf(os.Stderr,
				"This binary is a core. These are not meant to be executed directly.\n"+
	...
	protoVersion, protoType, coreSet := protocolVersion(opts)
	...
	if err != nil {
		logger.Error("core init error", "error", err)
		return
	}
	...
	if opts.TLSProvider != nil {
		...
		if err != nil {
			logger.Error("core tls init", "error", err)
			return
		}
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error preparing core: %s\n", err)
		os.Exit(1)
	}
	...
	if opts.Test != nil {
		...
		// connect via a core client, the output still gets swallowed.
	...
	logger.Debug("core address", "network", listener.Addr().Network(), "address", listener.Addr().String())
	...
	if opts.Test == nil {
		...
		go func() {
			...
			for {
				<-ch
				count++
				logger.Trace("core received interrupt signal, ignoring", "count", count)
			}
		}()
	}
...
func serverListener_tcp() (net.Listener, error) {
	...
	return nil, errors.New("couldn't bind core TCP listener")
}

func serverListener_unix(unixSocketCfg UnixSocketConfig) (net.Listener, error) {
	tf, err := os.CreateTemp(unixSocketCfg.socketDir, "core")
	if err != nil {
		return nil, err
	}
	path := tf.Name()
