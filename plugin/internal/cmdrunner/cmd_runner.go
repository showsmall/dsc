...
const unrecognizedRemotePluginMessage = `This usually means
  the core was not compiled for this architecture,
  the core is missing dynamic-link libraries necessary to run,
  the core is not executable by this process due to file permissions, or
  the core failed to negotiate the initial go-core protocol handshake
%s`
...
func (c *CmdRunner) Start(_ context.Context) error {
	c.logger.Debug("starting core", "path", c.cmd.Path, "args", c.cmd.Args)
	err := c.cmd.Start()
	if err != nil {
		return err
	}
	...
	c.logger.Debug("core started", "path", c.path, "pid", c.pid)
	return nil
}
