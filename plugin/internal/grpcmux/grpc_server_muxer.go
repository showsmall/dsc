...
// 与 core 包 GRPCBroker 的连接超时同一隐患语义：宿主与插件同时加载超大本地模型、
// 传输通道被挤占时首条连接可能迟到。受同一环境变量 PLUGIN_BROKER_CONN_TIMEOUT 控制。
var DefaultSessionTimeout = 5 * time.Second

// envConnTimeout 与 core.EnvConnTimeout 指向同一环境变量；
// grpcmux 为 internal 子包无法反向引用 core 包，故在此重复定义。
...
// GRPCServerMuxer implements the server (core) side of the gRPC broker's
