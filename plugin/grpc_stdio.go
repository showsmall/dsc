...
type grpcStdioClient struct {
	log         hclog.Logger
	stdioClient core.GRPCStdio_StreamStdioClient
}
...
func newGRPCStdioClient(
	...
	client := core.NewGRPCStdioClient(conn)

	// Connect immediately to the endpoint
	stdioClient, err := client.StreamStdio(ctx, &empty.Empty{})

	// If we get an Unavailable or Unimplemented error, this means that the core isn't
	// updated and linking to the latest version of go-core that supports
...
func (c *grpcStdioClient) Run(stdout, stderr io.Writer) {
	// This will be nil if stdio is not supported by the core
	...
	for {
		...
		var w io.Writer
		switch data.Channel {
		case core.StdioData_STDOUT:
			w = stdout

		case core.StdioData_STDERR:
			w = stderr
...
func copyChan(log hclog.Logger, dst chan<- []byte, src io.Reader) {
	...
	for {
		...
		if n > 0 {
			...
			// is no reader on the other side. We expect that go-core will
