...
type RPCServer struct {
	...
	// of our core system, we can't use the normal process values so we
...
func (s *RPCServer) Serve(lis net.Listener) {
	...
	for {
		...
		if err != nil {
			...
			log.Printf("[%s] core: core server: %s", severity, err)
			return
		}
...
func (s *RPCServer) ServeConn(conn io.ReadWriteCloser) {
	...
	if err != nil {
		_ = conn.Close()
		log.Printf("[ERR] core: error creating yamux server: %s", err)
		return
	}
	...
	if err != nil {
		_ = mux.Close()
		if err != io.EOF {
			log.Printf("[ERR] core: error accepting control connection: %s", err)
		}
	...
	for i := range stdstream {
		...
		if err != nil {
			_ = mux.Close()
			log.Printf("[ERR] core: accepting stream %d: %s", i, err)
			return
		}
	}
...
func (d *dispenseServer) Dispense(
	...
	p, ok := d.plugins[name]
	if !ok {
		return fmt.Errorf("unknown core type: %s", name)
	}
	...
	*response = id
	...
	// call returns. We wait for a connection for the core implementation
	// and serve it.
	go func() {
		conn, err := d.broker.Accept(id)
		if err != nil {
			log.Printf("[ERR] go-core: core dispense error: %s: %s", name, err)
			return
		}
...
func serve(conn io.ReadWriteCloser, name string, v interface{}) {
	...
	if err := server.RegisterName(name, v); err != nil {
		log.Printf("[ERR] go-core: core dispense error: %s", err)
		return
	}
