...
func TestClient(t *testing.T) {
	...
	// One error for connection refused, one for core exited.
...
func TestClient_SkipHostEnv(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.helper, func(t *testing.T) {
			...
			// Set env in the host process, which we'll look for in the core.
...
func TestClient_RequestGRPCMultiplexing_UnsupportedByPlugin(t *testing.T) {
	for _, name := range []string{
		"mux-grpc-with-old-core",
		"mux-grpc-with-unsupported-core",
	} {
		t.Run(name, func(t *testing.T) {
...
func TestClient_wrongVersion(t *testing.T) {
	process := helperProcess("test-proto-upgraded-core")
	c := NewClient(&ClientConfig{
		Cmd:              process,
		HandshakeConfig:  testHandshake,
		Plugins:          testGRPCPluginMap,
...
func TestClient_legacyClient(t *testing.T) {
	...
	// Ping, should work
	if err := client.Ping(); err == nil {
		t.Fatal("expected error, should negotiate wrong core")
	}
}
