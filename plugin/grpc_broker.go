...
type streamer interface {
	Send(*core.ConnInfo) error
	Recv() (*core.ConnInfo, error)
	Close()
}
...
// gRPCBrokerServer is used by the core to start a stream and to send
// connection information to/from the core. Implements GRPCBrokerServer and
// streamer interfaces.
type gRPCBrokerServer struct {
	core.UnimplementedGRPCBrokerServer

	// send is used to send connection info to the gRPC stream.
	send chan *sendErr

	// recv is used to receive connection info from the gRPC stream.
	recv chan *core.ConnInfo
...
func (s *gRPCBrokerServer) StartStream(stream core.GRPCBroker_StartStreamServer) error {
...
// to the client.
func (s *gRPCBrokerServer) Send(i *core.ConnInfo) error {
	ch := make(chan error)
	defer close(ch)
...
func (s *gRPCBrokerServer) Recv() (*core.ConnInfo, error) {
	select {
	case <-s.quit:
...
func newGRPCBrokerClient(conn *grpc.ClientConn) *gRPCBrokerClientImpl {
	return &gRPCBrokerClientImpl{
		client: core.NewGRPCBrokerClient(conn),
		send:   make(chan *sendErr),
		recv:   make(chan *core.ConnInfo),
		quit:   make(chan struct{}),
	}
...
// the quit channel is closed or the context reports Done. The stream will pass
// connection information to/from the core.
...
// Send is used by the GRPCBroker to pass connection information into the stream
// to the core.
func (s *gRPCBrokerClientImpl) Send(i *core.ConnInfo) error {
	ch := make(chan error)
	defer close(ch)
...
// Recv is used by the GRPCBroker to pass connection information that has been
// sent from the core to the broker.
func (s *gRPCBrokerClientImpl) Recv() (*core.ConnInfo, error) {
	select {
	case <-s.quit:
		return nil, errors.New("broker closed")
	case i := <-s.recv:
...
// streams between the core process and the host process.
//
// This allows a core to request a channel with a specific ID to connect to
...
func (b *GRPCBroker) Accept(id uint32) (net.Listener, error) {
	...
	err = b.streamer.Send(&core.ConnInfo{
		ServiceId: id,
...
func (b *GRPCBroker) AcceptAndServe(id uint32, newGRPCServer func([]grpc.ServerOption) *grpc.Server) {
	ln, err := b.Accept(id)
	if err != nil {
		log.Printf("[ERR] core: core acceptAndServe error: %s", err)
		return
	}
...
//
// It is possible for very long-running core hosts to wrap this value,
