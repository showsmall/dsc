...
// by core RPC implementations to change error behavior since you
...
// Client handles the lifecycle of a core application. It launches
// plugins, connects to them, dispenses interface implementations, and handles
// killing the process.
//
// Plugin hosts should use one Client for each core executable. To
// dispense a core type, use the `Client.Client` function, and then
...
// core and gets the address for the RPC connection.
//
...
// to reattach to the core process that this client started. This is
...
// start the core process if it isn't already started. Errors from
// starting the core are surpressed and ProtocolInvalid is returned. It
