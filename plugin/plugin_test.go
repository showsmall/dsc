...
// testPluginMap can be used for tests as a core map
var testPluginMap = map[string]Plugin{
	"test": new(testInterfacePlugin),
}

// testGRPCPluginMap can be used for tests as that need a GRPC core
...
func TestHelperProcess(*testing.T) {
	...
	case "test-skip-host-env-false":
		...
		os.Exit(1)
	case "mux-grpc-with-old-core":
		// gRPC broker multiplexing requested, but core has no awareness of the
		...
		<-make(chan int)
	case "mux-grpc-with-unsupported-core":
		// gRPC broker multiplexing requested, but core explicitly does not
