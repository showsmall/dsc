...
func newGRPCClient(doneCtx context.Context, c *Client) (*GRPCClient, error) {
	...
	cl := &GRPCClient{
		...
		doneCtx:    doneCtx,
		broker:     broker,
		controller: core.NewGRPCControllerClient(conn),
	}

	return cl, nil
}

// GRPCClient connects to a GRPCServer over gRPC to dispense core types.
type GRPCClient struct {
	...
	controller core.GRPCControllerClient
}

// Broker returns the internal GRPCBroker instance, allowing the host to
// register services and obtain IDs for core callbacks.
func (c *GRPCClient) Broker() *GRPCBroker {
	return c.broker
}
...
func (c *GRPCClient) Close() error {
	_ = c.broker.Close()
	_, _ = c.controller.Shutdown(c.doneCtx, &core.Empty{})
	return c.Conn.Close()
}
...
func (c *GRPCClient) Dispense(name string) (interface{}, error) {
	...
	p, ok := raw.(GRPCPlugin)
	if !ok {
		return nil, fmt.Errorf("core %q doesn't support gRPC", name)
	}
