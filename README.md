...
- **工具調用與插件化**：支持通過 Tool 插件擴展工具集，內置文件操作與 shell 執行能力。代碼中提供 `WorkspaceProtectionEnabled` 配置項，啟用時可限制模型文件操作在 workspace 根目錄內（默認即啟動 dsc 的目錄，也可經 `workspace_root` 絕對路徑覆蓋），防止路徑遍歷攻擊；默認情況下保護機制已啟用。
